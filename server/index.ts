...
if (!sportsApiKey) {
  console.warn(
    `[server] WARNING: No Statpal API key found for "${sportsProvider}". Sports data endpoints will return empty. Set STATPAL_API_KEY.`,
  );
} else if (sportsApiKeyEnv && sportsApiKeyEnv !== 'STATPAL_API_KEY') {
  console.warn(
    `[server] WARNING: Using legacy Statpal key env "${sportsApiKeyEnv}". Prefer STATPAL_API_KEY.`,
  );
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (isApi) {
      ...
      if (!dbReady) {
        if (url.pathname === '/api/casino/config' && req.method === 'GET') {
          if (await handleCasinoRoutes(safePool, req, res, url)) return;
        }
