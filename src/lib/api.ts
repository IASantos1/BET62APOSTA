...
export async function apiFootballRequest(endpoint: string, sport: string = 'football'): Promise<any> {
  const params = new URLSearchParams({
    sport,
    endpoint,
  });
  const fullUrl = `/api/sports/api-football-proxy?${params.toString()}`;
  const cacheKey = getCacheKey(fullUrl);
  
  // ✅ VERIFICAR CACHE LOCAL PRIMEIRO
  const cached = getFromLocalCache(cacheKey);
  if (cached) {
    console.log(`📦 Cache local HIT: ${sport} → ${endpoint}`);
    return cached;
  }
  
  // ✅ VERIFICAR THROTTLE
  const throttleCheck = shouldThrottleRequest();
  if (throttleCheck.throttle) {
    // ✅ Retornar cache expirado se disponível
    const expiredCache = localCache.get(cacheKey);
    if (expiredCache) {
      console.log(`📦 Usando cache expirado durante throttle`);
      return expiredCache.data;
    }
    
    // ✅ Esperar menos tempo
    const reducedWait = Math.min(throttleCheck.waitMs, 500);
    await new Promise(resolve => setTimeout(resolve, reducedWait));
  }
  
  try {
    console.log(`📡 [API-Football Proxy] ${sport} → ${endpoint}`);
    ...
    if (data.rateLimit || (data.errors && isRateLimitError(data.errors))) {
      handleRateLimitError();
    ...
    // ✅ CACHE AGRESSIVO
    // TTL: 10s para live, 60s para teams/leagues, 30s para outros
    let cacheTTL = 30000;
    if (endpoint.includes('live') || endpoint.includes('fixtures?live=all')) {
      cacheTTL = 10000; // 10s para dados ao vivo
    } else if (endpoint.includes('teams') || endpoint.includes('leagues')) {
      cacheTTL = 6 * 60 * 60 * 1000; // 6 horas para dados estáticos
    }
    
    saveToLocalCache(cacheKey, data, cacheTTL);
...
export async function oddsApiRequest(
  ...
  const cacheKey = getCacheKey(`${endpoint}?${queryParams.toString()}`);
  
  // ✅ VERIFICAR CACHE LOCAL PRIMEIRO
  const cached = getFromLocalCache(cacheKey);
  if (cached) {
    console.log(`📦 Cache local HIT: The Odds API → ${endpoint}`);
    return cached;
  }
  ...
  try {
    ...
    if (endpoint === '/sports') {
      console.log('📡 [Odds Backend] /odds/sports', Object.fromEntries(queryParams.entries()));
      data = await apiFetch('/odds/sports', { method: 'GET' });
    } else if (endpoint.startsWith('/sports/') && endpoint.endsWith('/odds')) {
      console.log('📡 [Odds Backend] Endpoint de odds ainda não implementado, devolvendo vazio:', endpoint);
      data = [];
    } else if (endpoint.startsWith('/sports/') && endpoint.endsWith('/scores')) {
      console.log('📡 [Odds Backend] Endpoint de scores ainda não implementado, devolvendo vazio:', endpoint);
      data = [];
    } else {
      data = [];
    }
    ...
    const cacheTTL = endpoint.includes('scores') ? 15000 : 20000;
    saveToLocalCache(cacheKey, data, cacheTTL);

    console.log(
      `✅ Odds Backend: ${Array.isArray(data) ? data.length : 'N/A'} resultados (cache: ${
        cacheTTL / 1000
      }s)`,
    );
